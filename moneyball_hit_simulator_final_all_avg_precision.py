...
# --- Title ---
st.image("https://i.imgur.com/h4fFTFY.png", width=160)  # Moneyball Phil Logo
...
with st.form("player_input_form"):
    ...
    season_avg = st.number_input("Season AVG", min_value=0.0, max_value=1.0, step=0.0001)
    last7_avg = st.number_input("Last 7 Days AVG", min_value=0.0, max_value=1.0, step=0.0001)
    split_avg = st.number_input("Home/Away or vs AL/NL AVG", min_value=0.0, max_value=1.0, step=0.0001)
    pitcher_avg = st.number_input("Pitcher AVG vs", min_value=0.0, max_value=1.0, step=0.001, format="%0.3f")
    ...
    st.markdown(f"**Weighted Batting Average:** `{round(weighted_avg, 3)}`")
