...
with st.form("player_input"):
    ...
    st.markdown(f"**Weighted AVG:** `{calculate_weighted_avg(season_avg, last7_avg, split_avg, hand_avg, pitcher_avg)}`")
...
if len(st.session_state.players) >= 2:
    ...
    if len(selected) in [2, 3]:
        ...
        st.markdown(f"**Implied Parlay Probability (from odds):** {implied_parlay_prob:.2%}")
